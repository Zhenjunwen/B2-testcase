...
def cancel_order(token_wen,order_id):
    #商户下架广告
    url = "%s/api/v1/otc/cancel_order"%B4_url
    body = {
        "token":token_wen,
        "order_id":order_id
    }
...
def get_orders(token_wen,state="7",pay_way="0",nationality="0",base_currency="",quote_currency=""):
    #获取商户发布的广告
    url = "%s/api/v1/otc/get_orders"%B4_url
    body = {
        "token":token_wen,
        "page_number":"2",
        "page_size":"10",
...
def user_sell_transaction(token_junxin,order_id,amount,price):
    # 客户下卖单
    url = "%s/api/v1/otc/add_transaction"%B4_url
...
def usersell_business_confirm(token_wen,trx_id):
    # 买币广告-商户确认付款
    url = "%s/api/v1/otc/usersell_business_confirm" % B4_url
    body = {
        "token": token_wen,
        "trx_id": trx_id
    }
...
def usersell_customer_confirm(token_wen,trx_id):
    # 买币广告-客户确认收款
    url = "%s/api/v1/otc/usersell_customer_confirm" % B4_url
    body = {
        "token": token_wen,
        "trx_id": trx_id
    }
...
def usersell_business_cancel(token_wen,trx_id):
    # 买币广告-商户取消交易
    url = "%s/api/v1/otc/usersell_business_cancel" % B4_url
    body = {
        "token": token_wen,
        "trx_id": trx_id
    }
...
def user_customer_confirm_transaction(token_junxin,trx_id):
    # 客户下买单确认付款
    url = "%s/api/v1/otc/customer_confirm_transaction"%B4_url
    user_sell_pay_detail = json.dumps(sell_payway_detail(token_wen)[0])
    body = {
        "token": token_junxin,
        "trx_id": trx_id,
...
def business_confirm_transaction(token_wen,trx_id):
    # 卖币广告-商户确认收款
    url = "%s/api/v1/otc/business_confirm_transaction" % B4_url
    body = {
        "token": token_wen,
        "trx_id": trx_id
    }
...
def cancel_transaction(token_junxin,trx_id):
    # 卖币广告-客户取消交易
    url = "%s/api/v1/otc/cancel_transaction" % B4_url
    body = {
        "token": token_junxin,
        "trx_id": trx_id
    }
...
def admin_cancel_usersell_transaction(sys_token,trx_id):
    ...
    url = "%s/api/v1/admin/otc/cancel_usersell_transaction" % B4_url
    body = {
        "token": sys_token,
        "trx_id": trx_id
    }
...
def admin_confirm_usersell_transaction(sys_token,trx_id):
    ...
    url = "%s/api/v1/admin/otc/confirm_usersell_transaction" % B4_url
    body = {
        "token": sys_token,
        "trx_id": trx_id
    }
...
def admin_cancel_arbitrated_transaction(sys_token,trx_id):
    ...
    url= "%s/api/v1/admin/otc/cancel_arbitrated_transaction" % B4_url
    body = {
        "token": sys_token,
        "trx_id": trx_id
    }
...
def admin_confirm_arbitrated_transaction(sys_token,trx_id):
    ...
    url = "%s/api/v1/admin/otc/confirm_arbitrated_transaction" % B4_url
    body = {
        "token": sys_token,
        "trx_id": trx_id
    }
...
def arbitrate_transaction(token,trx_id):
    # 卖家申诉
    url = "%s/api/v1/otc/arbitrate_transaction" % B4_url
    body = {
        "token": token,
        "trx_id": trx_id
    }
...
def query_invitation_get_stats(token):
    #我的邀请信息
    url = "%s/api/v1/partner/query_invitation_get_stats" % B4_url
    body = {
        "token":token
    }
    run = RunMain(url=url, params=None, data=body,
...
def query_identity_byToken(token):
    #查询用户是否是荣誉合伙人
    url = "%s/api/v1/partner/query_identity_byToken" % B4_url
    body = {
        "token":token
    }
    run = RunMain(url=url, params=None, data=body,
...
def otc_add_deposit(token,symbol,amount):
    #1192-商户追加保证金
    url = "%s/api/v1/otc/add_deposit" % B4_url
    body={
        "token":token,
        "symbol":symbol,
        "amount":amount
