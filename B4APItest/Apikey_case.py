...
from B4APItest.login_register import send_email_sms,send_sms
...
def apikey_create(account,token,permission,bind_ip,remark,dialing_code="86"):
    #申请ApiKey
...
def apikey_get_list(token):
    #获取ApiKey列表
    url = "%s/api/v1/merchant/apikey/get_list" % B4_url
    body={
        "token":token,
    }
    run = RunMain(url=url, params=None, data=body,
...
def apikey_modify(account,token,api_key,bind_ip,remark,permission,dialing_code="86"):
    #修改ApiKey信息
    url = "%s/api/v1/merchant/apikey/modify" % B4_url
    db = DB(host, port, user, password, database)  # B4devDB
...
def apikey_delete(token,api_key):
    #删除ApiKey
    url = "%s/api/v1/merchant/apikey/delete" % B4_url
    body={
        "token":token,
        "api_key":api_key
    }
...
if __name__ == "__main__":
    # apikey_create(account="15521057551",token=token_wen, permission="7",bind_ip="", remark='ceshi')
    ...
    # apikey_modify(account="15521057551", token=token_wen, api_key="a721bfb0-8264-4233-918d-83652d748354", bind_ip="", remark="编辑APIKEY", permission="7", dialing_code="86")
    ...
    # apikey_get_list(token=token_wen)
    # apikey_delete(token=token_wen, api_key="a721bfb0-8264-4233-918d-83652d748354")
    pass
